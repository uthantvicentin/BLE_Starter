...
class ScanResultAdapter(
    ...
    class ViewHolder(
        ...
        fun bind(result: ScanResult) {
            val r0 = 63.0
            Timber.i(String.format(result.rssi.toString()))
            var distance= 10.0.pow((r0 - result.rssi / (2.0 * 10.0)))
            val df = DecimalFormat("#.####")
            df.roundingMode = RoundingMode.CEILING
            distance /= 10.0.pow(r0+3)
            println(df.format(distance)+"m")
            //println(distance)
            ...
            view.distancia.text =  df.format(distance) + "m"
